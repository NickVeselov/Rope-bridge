...
namespace octet { namespace helpers {
  ...
  public:
    ...
    void update(scene_node *player_node, scene_node *camera_node) {
        ...
        player_node->clamp_linear_velocity(velocity);

        float friction = 0.0f;
        if (the_app->is_key_down('A')) {
          player_node->activate();
          player_node->apply_central_force(camera_node->get_x() * (-velocity));
        } else if (the_app->is_key_down('D')) {
          player_node->activate();
          player_node->apply_central_force(camera_node->get_x() * (+velocity));
        } else if (the_app->is_key_down('W')) {
          player_node->activate();
          player_node->apply_central_force(camera_node->get_z() * (-velocity));
        ...
        if (the_app->is_key_going_down(' ')) {
          player_node->apply_central_force(camera_node->get_y() * (+10*velocity));
        }
        player_node->set_friction(friction);

        mat4t &camera_to_world = camera_node->access_nodeToParent();
        camera_to_world.w() = (player_node->get_position() + camera_translation ).xyz1();
      #endif
    }
  };
}}
